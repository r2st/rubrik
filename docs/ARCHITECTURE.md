...
| **TOTP backup codes** | `api/admin/totp.py` + `POST /admin/totp/backup-codes/regenerate` | 8 single-use `XXXX-XXXX` codes minted at setup, SHA-256 hashed as the `auth.admin_totp_backup_codes` secret-typed setting. Substitute for a TOTP code on login (constant-time match, atomic consume). Recovery without shell access. |
| **Per-tenant isolation** | `src/tenant.py` ContextVar + `_TenantContextMiddleware` (pure ASGI) | Tenant id derived per request from JWT claims (`tid`/`tenant`/`tenant_id`) > hashed `X-API-Key`. `DatabaseRepository.get` auto-scopes by tenant; `CachedTranscriptRepository` keys entries `{tenant}:{meeting_id}` so cache hits can't cross tenants. |
...
| **GDPR right-to-be-forgotten** | `api/admin/gdpr.py` + `POST /admin/gdpr/delete-customer` | Atomic delete from `meetings` + `customer.deleted` outbox event + hashed audit-log row + cache invalidation. **Structured JSONB match** on `raw->'info'->>'customer'` (Postgres) / `json_extract` (SQLite) — never a free-text LIKE. Audit row never carries the raw customer name. Operator runbook: `deploy/gdpr-runbook.md`. |
| **Streaming bulk export + quotas** | `GET /admin/meetings/export.ndjson` | Repository `stream()` iterator → NDJSON via `StreamingResponse`. Constant memory at 100M+ rows. Gated by the admin-write rate limit AND a per-actor per-UTC-day cap (`export.max_per_day`, default 4). |
| **Snapshot HMAC integrity** | `api/snapshot.py` + `admin.snapshot_signing_secret` (bootstrap-only, file-mounted) | HMAC-SHA256-signs the manifest at write; read refuses unsigned or signature-mismatched manifests once a key is configured. Closes the pickle-RCE primitive an unsigned S3 snapshot creates. Signing key is *not* a runtime setting — a compromised admin can't disable it. |
...
| **CORS** | `api/main.py::_safe_cors_origins` | Default `auth.cors_origins=[]`. In prod, `"*"` is stripped + a warning logged when `allow_credentials=True`. |
| **robots.txt + RFC 9116** | `GET /robots.txt` + `GET /.well-known/security.txt` | Disallow crawlers from `/admin` + `/api/v1/admin`. Point researchers at the disclosure channel. |
| Audit log | `audit_log` table | Every admin mutation recorded with actor + before/after value + `ip_address` + `user_agent` (migration 0005). `secret`-typed values are masked. |
| Outbox + DLQ + reaper | `OutboxEvent` table + `DLQPublisher` + `POST /admin/outbox/{replay,reap}` | At-least-once event fan-out without dual-writes. Poison rows route to DLQ. Background reaper deletes processed rows older than `outbox.reap_processed_days` (default 7d); operator override at `POST /admin/outbox/reap`. Trace IDs from `emit()` propagate into Kafka `traceparent` headers. |
